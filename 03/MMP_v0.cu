...
__constant__ double3 AVG[MAX_COUNT_OF_CLASSES];
__constant__ double COV[MAX_COUNT_OF_CLASSES * MATRIX_SIZE];
__constant__ double LOG_DET_COV[MAX_COUNT_OF_CLASSES];
__constant__ double INVERSED_COV[MAX_COUNT_OF_CLASSES * MATRIX_SIZE];
struct Im {
    int w, h;
    uchar4 *arr;
};
...
double *calc_cov(Im &data, int **arr, int nc, double3 *avg) {
    ...
    for(int class_ = 0; class_ < nc; ++class_) {
        ...
        for(int j = 1; j <= 2 * count; j += 2) {
            ...
            // std::cout << "(" << avg[class_].x << "," << avg[class_].y << "," << avg[class_].z << ")\n";
...
void inverse_matrix_(double *matrix, const double *src, int nc) {
    ...
    std::cout << "Inversed matrix cov: " << nc + 1 << '\n';
    for(int i = 0; i < 3; ++i) {
        for(int j = 0; j < 3; ++j) {
            std::cout.precision(5);
            std::cout << std::fixed << matrix[nc * 9 + i * 3 + j]  << ' ';
        }
        std::cout << '\n';
    }
}
...
double calc_tmp(double3 p, int j, double3 *avg, double* inversed_matrix, double* log_det_cov) {
    double3 pix;
    pix.x = p.x - avg[j].x;
    pix.y = p.y - avg[j].y;
    pix.z = p.z - avg[j].z;
    double3 tmp;
    tmp.x = pix.x * inversed_matrix[j * 9 + 0] +
            pix.y * inversed_matrix[j * 9 + 3] + 
            pix.z * inversed_matrix[j * 9 + 6];
    tmp.y = pix.x * inversed_matrix[j * 9 + 1] +
            pix.y * inversed_matrix[j * 9 + 4] + 
            pix.z * inversed_matrix[j * 9 + 7];
    tmp.z = pix.x * inversed_matrix[j * 9 + 2] +
            pix.y * inversed_matrix[j * 9 + 5] + 
            pix.z * inversed_matrix[j * 9 + 8];
    double res =  - (tmp.x * pix.x + tmp.y * pix.y + tmp.z * pix.z) - log_det_cov[j];
    return res;
}
void linear(Im &img, int nc, int size, double* inversed_cov, double3* avg, double* log_det_cov) {
    for(int i = 0; i < size; ++i) {
        uchar4 p = img.arr[i];
        double3 pix;
        pix.x = p.x;
        pix.y = p.y;
        pix.z = p.z;
        double max = calc_tmp(pix, 0, avg, inversed_cov, log_det_cov);
        double class_ = 0;
        for(int j = 1; j < nc; ++j) {
            double curr = calc_tmp(pix, j, avg, inversed_cov, log_det_cov);
            if(curr > max) {
                max = curr;
                class_ = j;
            }
...
// this
    ...
    print_cov(cov, nc);
#endif
    CSC(cudaMemcpyToSymbol(AVG, avg, sizeof(double3) * nc));
    CSC(cudaMemcpyToSymbol(COV, cov, sizeof(double) * MATRIX_SIZE * nc));
    CSC(cudaMemcpyToSymbol(LOG_DET_COV, log_det_cov, sizeof(double) * nc));
    CSC(cudaMemcpyToSymbol(INVERSED_COV, inversed_cov, sizeof(double) * MATRIX_SIZE * nc));

    uchar4 *img;
    CSC(cudaMalloc(&img, sizeof(uchar4) * data.w * data.h));
    CSC(cudaMemcpy(img, data.arr, sizeof(uchar4) * data.w * data.h, cudaMemcpyHostToDevice));
    kernel<<<1024, 1024>>>(img, nc, data.w * data.h);
    CSC(cudaDeviceSynchronize());
    CSC(cudaGetLastError());
    CSC(cudaMemcpy(data.arr, img, data.w * data.h * sizeof(uchar4), cudaMemcpyDeviceToHost));
    Im log;
    read_img(log, path);
    linear(log, nc, data.w * data.h, inversed_cov, avg, log_det_cov);
    write_img(log, "log.out");
