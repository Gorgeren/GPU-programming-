...
#ifndef checker
        img[idx].w = 255;
        img[idx].x = (unsigned char)AVG[class_].x;
        img[idx].y = (unsigned char)AVG[class_].y;
        img[idx].z = (unsigned char)AVG[class_].z;
#endif // checker
        idx += offset;
    }
}
...
int main() {
    ...
    for(int i = 1; i <= 1024; i *= 2) {
        for (int j = 32; j <= 1024; j *= 2) {
            ...
            kernel<<<i, j>>> (img, nc, data.w * data.h);
            ...
            std::cout << "Grid =  "<< i << "Block size = " << j << " time = " << t << " ms" << std::endl;
        }
    }
    std::cout << "min is = " << min << "ms for Grid size = " << min_i << " Block size = " << min_j << '\n';
#else
